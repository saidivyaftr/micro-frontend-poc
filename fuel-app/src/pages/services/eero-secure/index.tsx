...
import EeroSecureDetails from 'src/libs/services/eero-secure/EeroSecureDetails'
...
function SelfServicePDP(props: OfferProps): JSX.Element {
  ...
  const pageCode = SELF_SERVICE.PRODUCT_FSWFI_PAGE_CODE
  ...
  const checkIfValidOffer = (cartData: offersData) => {
    const eeroSecureProduct = cartData?.Offers.find(
      (item: offerData) => item?.ItemCode === pageCode,
    )
    if (!eeroSecureProduct) {
              ...
              {!showSpinner ? (
                <CartContextProvider>
                  <SelfServiceHeader
                    ...
                    clickEventName={SELF_SERVICE.PRODUCT_EEROSECURE}
...
export const getStaticProps = customStaticProps(AppRoutes.SelfServiceEeroSecure)
