      ...
      <ComponentWrapper>
        <LeftWrapper>
          {dynamicComponents?.map((compData: any, i: number) => {
            // TODO: Implement the Auto pay and paperless into a separate component instead of injecting into ActionCard
            // if (
            //   compData?.componentName === 'actionCard' &&
            //   userAccountStatus != undefined &&
            //   (compData?.fields.data.datasource.title?.value === 'Auto Pay' ||
            //     compData?.fields.data.datasource.title?.value ===
            //       'Paperless Billing')
            // ) {
...
const ignoredArticlePaths = ['tutorials']
...
export const getStaticPaths = async () => {
  ...
  try {
    ...
    paths = category.reduce(
      (
        path: any,
        { name: category, article = [], subcategory = [] }: any = {},
      ) => {
        const articlePaths = article
          ?.filter(
            ({ name: articlePath }: any = {}) =>
              !ignoredArticlePaths.includes(articlePath),
          )
          ?.map(({ name: articlePath }: any) => ({
            params: { category, articlePath: [articlePath] },
          }))
        const subCategoriesPaths = subcategory?.reduce(
          (articles: any, { name, article = [] }: any) => {
            const newArticles = article?.map(({ name: articlePath }: any) => {
              const params = { category, articlePath: [name, articlePath] }
              return { params }
            })
        ...
        return [...path, ...articlePaths, ...subCategoriesPaths]
      },
      [],
    )
  } catch {
