...
export default async function addPhoneNumber(
  ...
  try {
    ...
    const { uuid, phoneNumber, givenName, familyName, isPrimary } =
      decryptPayload(req.body.d)
    const endpoint = `/myfrontier/v3/accounts/authorized/${uuid}/phonenumber`
    ...
    apiErrorLogger(error, 'contact-verification-add-phone')
    return res
