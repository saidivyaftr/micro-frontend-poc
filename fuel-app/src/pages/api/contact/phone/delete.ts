...
export default async function deletePhoneNumber(
  ...
  try {
    ...
    const { uuid, phoneId } = decryptPayload(req.headers.d)
    const endpoint = `myfrontier/v2/accounts/${uuid}/phonenumbers/${phoneId}`
    ...
    apiErrorLogger(error, 'contact-verification-delete-phone')
    return res
