...
export default async function updateEmail(
  ...
  try {
    ...
    const { uuid, emailId, emailAddress, givenName, familyName, isPrimary } =
      decryptPayload(req.body.d)

    const endpoint = `myfrontier/v3/accounts/authorized/${uuid}/emailaddress/${emailId}`
    ...
    apiErrorLogger(error, 'contact-verification-update-email')
    return res
